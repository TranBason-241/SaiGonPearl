...
var ggAPI = "";
var _linkSheet =
  "https://docs.google.com/spreadsheets/d/16ziy7hNuGV5FT2NOX8Oi46W5wMZdvAT-NuY_kPdil8g/edit#gid=0";
...
$(document).ready(function () {
  ...
  var _isMobile = /Android|iPhone|iPad|iPod|BlackBerry/i.test(
...
btnNavs = document.querySelectorAll(".btn-nav");
...
btnNavs.forEach((element) => {
  element.addEventListener("click", (e) => {
    ...
    document.querySelector(".schedule-img.active").classList.remove("active");
    // change button & img
    e.target.classList.add("active");
    imgList = document
      .querySelectorAll(".schedule-img")
...
const scrollButtons = document.querySelectorAll(".btn-register");
...
document.querySelector(".btn-test").addEventListener("click", () => {
  $("#exampleModal").modal("show"); // Hiển thị modal khi kết quả là true
});

// xử lí click ra ngoài
...
function getHref() {
  ...
  $(".share-fb").attr(
    "href",
    "https://www.facebook.com/sharer/sharer.php?u=" + _href
  );
...
function onNext(e) {
  ...
  if (checkRequired(_current) == true) {
    if (
      _current == 3 &&
      _data["Ngành nghề kinh doanh chính"] == "Công nghiệp chế biến, chế tạo"
    ) {
      _nextID = 4;
    }
...
function onPrev(e) {
  ...
  if (
    _current == 3 &&
    _data["Bạn có biết một trong các nội dung sau đây không?"] == "Không"
  ) {
    _prevID = 1;
  }
  ...
  if (
    _current == 8 &&
    _data["Ngành nghề kinh doanh chính của doanh nghiệp bạn"] ==
...
function checkRequired(currentPage) {
  let _numItem = $(
    "#listQuestion-" + currentPage + ".show .card:not(.hide)"
  ).length;
  let _numReq = $(
    "#listQuestion-" + currentPage + ".show .card.as-done:not(.hide)"
  ).length;
  ...
  if (_numItem == _numReq) {
    ...
    $.each(
      $(
        "#listQuestion-" + currentPage + ".show .card:not(.as-done):not(.hide)"
      ),
      function (index, ele) {
        if (index == 0) {
